#include "Unit1OpenGLInit.h"

#include <QDebug>

Unit1OpenGLInit::~Unit1OpenGLInit()
{
    //QOpenGLWidget接口:通过使相应的上下文成为当前上下文并在该上下文中绑定帧缓冲区对象，
    ...
    //QOpenGLWidget接口:释放上下文。在大多数情况下不需要调用此函数，
    //因为小部件将确保在调用paintGL()时正确绑定和释放上下文。
    doneCurrent();
}

void Unit1OpenGLInit::initializeGL()
{
    //QOpenGLFunctions接口:为当前上下文初始化OpenGL函数解析
    initializeOpenGLFunctions();

    //[]初始化顶点数组对象
    //（create*分配对象名称，bind*绑定对象为OpenGL状态机当前对象）
    //分配顶点数组对象名称vao
    //void glCreateVertexArrays(GLsizei n​, GLuint *arrays​);
    //旧版本用void glGenVertexArrays(GLsizei n​, GLuint *arrays​);
    //参数1为生成名称数量，参数2为顶点数组
    //返回的vao可以用来分配多个缓存对象
    ...
    //绑定一个命名的缓冲区对象（用glNamedBufferStorage可以直接指定buffer，就不bind了）
    //void glBindBuffer(GLenum target​, GLuint buffer​);
    //参数1缓冲对象类型，参数2缓冲对象名称
    //OpenGL有很多缓冲对象类型，顶点缓冲对象的缓冲类型是GL_ARRAY_BUFFER
    //OpenGL允许我们同时绑定多个缓冲，只要它们是不同的缓冲类型
    //（虽然glNamedBufferStorage不需要，但是glVertexAttribPointer也需要先bind）
    ...
    //旧版本用glNamedBufferData(GLuint buffer,GLsizeiptr size,const void * data,GLbitfield flags);
    //参数1缓冲对象，参数2数据字节大小，参数3数据指针，没数据则为NULL
    //参数4指定缓冲区的数据存储的预期用途
    //或者用指定target缓冲类型的版本（glBindBuffer+glBufferStorage）
    //void glBufferStorage(GLenum target,GLsizeiptr size,const void * data,GLbitfield flags);
    //旧版本用glBufferData(GLenum target​, GLsizeiptr size​, const GLvoid * data​, GLenum usage​);
...
void Unit1OpenGLInit::paintGL()
{
    ...
    static const float back[] = { 1.0f, 0.0f, 0.0f, 0.0f };
...
void Unit1OpenGLInit::resizeGL(int width, int height)
{
    //设置视口，缩放时是靠窗口左下角缩放
